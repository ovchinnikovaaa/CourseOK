...
public class CreateNoteActivity extends BaseNoteActivity {
    ...
    //поле для файла, куда камера будет сохранять изображение
    ...
    /*метод для обработки клика на элемент меню, в котором будет вызван метод saveNote()*/
    @Override
    ...
    // получаем результат выбора изображения из галереи
    @Override
    ...
    /**
     * Метод для сохранения конспектов
     */
    ...
    //метод показывает диалог выбора изображения
    private void showImageSelectionDialog() {
        AlertDialog alertDialog = new AlertDialog.Builder(this)
                .setTitle(R.string.title_dialog_attachment_variants) //устанавливаем заголовок диалога
                .setItems(R.array.attachment_variants, new DialogInterface.OnClickListener() { //устанавливаем варианты
                    @Override
                    // смотрим что выбрал пользователь и вызываем соответствующий метод
        ...
        // проверка "завершаемости" активности
        if (!isFinishing()) {
            alertDialog.show();
        }
    }
    ...
    private File createImageFile() {
        ...
        // Выглядит она примерно так: /sdcard/Android/data/com.test.myapplicationfinal/files/Pictures
    ...
    private void pickImageFromGallery() {
        Intent intent = new Intent(Intent.ACTION_PICK); // Загружает дочернюю Активность, позволяющую выбрать элемент из источника данных
    ...
    private void takePhoto() {
        ...
        if (currentImageFile != null) {
            ...
            Uri imageUri = FileProvider.getUriForFile(this,
                    "com.test.myapplicationfinal.fileprovider",
                    currentImageFile);
    ...
    private void addImageToDatabase(File file) {
        if (noteId == -1) {
            // аттачи только в режиме редактирования
            return;
        }
    ...
    /*метод, в котором мы будем удалять изображение*/
    ...
    private final NoteImagesAdapter.OnNoteImageLongClickListener onNoteImageLongClickListener =
            new NoteImagesAdapter.OnNoteImageLongClickListener() {
                ...
                public void onImageLongClick(final long imageId) {
                    /*диалог, в котором спросим пользователя, действительно ли надо удалить изображение*/
   ...
   /**
    *  Метод для отправления конспектов
    */
    public void onSendNote(View view) {
        EditText messageView = findViewById(R.id.text_et);
